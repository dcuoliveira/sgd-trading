...
```{r}
ggplot(data = signals %>% select(date, residual) %>% filter(date >= start_date) %>% melt("date"), 
...
ggplot(data = signals %>% filter(date >= start_date) %>% melt("date"), 
                   mapping = aes(x = date, y = value, colour = variable, group = variable)) +
              geom_line() +
              scale_x_date(date_breaks = "1 year") + 
              theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
```{r}
ggplot(data = coefs %>% filter(date >= start_date) %>% melt("date"), 
                   mapping = aes(x = date, y = value, colour = variable, group = variable)) +
              geom_line() +
              scale_x_date(date_breaks = "1 year") + 
              theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
```{r}
ggplot(data = positions %>% filter(date >= start_date) %>% melt("date"), 
                   mapping = aes(x = date, y = value, colour = variable, group = variable)) +
              geom_line() +
              scale_x_date(date_breaks = "1 year") + 
              theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
# each asset
ggplot(data = portfolio_cumm_rets %>% filter(date >= start_date) %>% melt("date"), 
                   mapping = aes(x = date, y = value, colour = variable, group = variable)) +
              geom_line() +
              scale_x_date(date_breaks = "1 year") + 
              theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
list(mean=mean(ret_vec) * 48,
     vol=sd(ret_vec) * sqrt(48),
     SR=(mean(ret_vec) / sd(ret_vec)) * sqrt(48))
```
...
mutate(vol=roll_sd(x = rets, width = 12) * sqrt(48) * 100) %>%
