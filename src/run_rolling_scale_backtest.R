...
option_list <- list(
  ...
  make_option(c("--strategy_type"), type = "character", help = "Strategy type", default = "rolling_scale"),
  make_option(c("--window_size"), type = "integer", help = "Window size", default = 52 * 10),
  make_option(c("--ma_window_size"), type = "integer", help = "Moving Average Window Size", default = 52 * 2),
  ...
  make_option(c("--threshold"), help = "Threshold to the used on the strategy", default = 0.5)
)
...
if (MODEL == "dlm"){
  ...
  mean <- rollapply(residuals_df$residual, width = MA_WINDOW_SIZE, FUN = mean, align = "right", partial = TRUE)
