...
for (i in seq_len(n_test)) {
  ...
  # predict with var
  pred_var <- predict(var_model, n.ahead = h)
  ...
  for (fwd in FWD_NAMES) {
    ...
    forecasts_list[[fwd]][i, ] <- pred_var[horizon_idx, ]
...
plot_data <- summary_df %>%
  select(Tenor, Currency, VAR_RMSE_Ratio, FWD_RMSE_Ratio) %>%
  pivot_longer(cols = c(VAR_RMSE_Ratio, FWD_RMSE_Ratio),
...
ggplot(plot_data, aes(x = Tenor, y = RMSE_Ratio, color = Model, group = Model)) +
  ...
  labs(
    title = "RMSE Ratio (VAR / RW) per Currency",
    y = "RMSE Ratio",
    x = "Tenor"
  ) +
  theme_minimal() +
