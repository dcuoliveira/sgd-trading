...
ewma_outputs <- readRDS(file.path(OUTPUT_PATH, model, paste0("backtest_", strategy, "_results.rds")))
...
roll_portfolio_vol <- roll_sd(x = ewma_ret_df$portfolio, width = 90) * sqrt(12) * 100
...
print(paste0("Sharpe ratio of ", as.character(round(mean(vol_adj_ret_df$portfolio, na.rm = TRUE) / sd(vol_adj_ret_df$portfolio, na.rm = TRUE) * sqrt(252), 2))))
...
ewma_stats <- list(mean=mean(ewma_portfolio_rets_df$portfolio) * 252,
                   vol=sd(ewma_portfolio_rets_df$portfolio) * sqrt(252),
                   SR=(mean(ewma_portfolio_rets_df$portfolio) / sd(ewma_portfolio_rets_df$portfolio)) * sqrt(252))
