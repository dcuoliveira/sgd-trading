...
for (i in seq_len(n_test)) {
  ...
  # estimate and predict h steps ahead for each ccy using lm()
  pred_ols <- list()
  for (ccy in SPOTS){
    ols_model <- lm(
    ...
    tmp_pred_ols <- predict(ols_model)
    
    # select last row of predictions
    tmp_pred_ols_val <- tail(tmp_pred_ols, n = 1)
    
    tmp_pred_ols <- data.frame(ccy = rep(tmp_pred_ols_val, h)) %>%
      rename(!!paste0(ccy, ' Curncy') := ccy)
    row.names(tmp_pred_ols) <- dim(current_data)[1] + seq(1, h)
  ...
  for (fwd in FWD_NAMES) {
    ...
    forecasts_list[[fwd]][i, ] <- pred_ols[horizon_idx, ]
