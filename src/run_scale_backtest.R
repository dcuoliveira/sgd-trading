...
n <- length(cointegration_error_df$residual)
sd_data <- roll_sd(cointegration_error_df$residual, width = WINDOW_SIZE, min_obs = WINDOW_SIZE)
cointegration_error_df <- cointegration_error_df %>%
  drop_na() %>%
  mutate(residual_vol=sd_data) %>%
  mutate(ub=THRESHOLD*residual_vol, lb=-THRESHOLD*residual_vol) %>%
  select(-residual_vol) %>%
  as.data.table() %>%
  tidyr::drop_na()
